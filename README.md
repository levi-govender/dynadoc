...
`evaluate(expr, answers)` in `src/lib/expr/evaluate.ts` is a pure boolean walk of `eq` / `in` / `exists` / `not` / `and` / `or`. Missing keys are false for `eq` and `exists`. Unknown operators throw `UnknownOperatorError`. The document resolver is a later ticket.
